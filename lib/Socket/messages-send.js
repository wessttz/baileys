    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, AI = false }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                const rawParticipantsList = groupData ? groupData.participants.map(p => p.id) : [];
                let participantsList = rawParticipantsList;
                const groupAddressingModeForParticipants = groupData?.addressingMode || 'lid';
                if (isGroup && groupAddressingModeForParticipants === 'lid' && meLid) {
                    const pnParticipants = rawParticipantsList.filter(id => isPnUser(id) || isHostedPnUser(id));
                        ...
                        try {
                            ...
                            for (const mapping of (lidMappings || [])) {
                                if (mapping.pn && mapping.lid) {
                                    pnToLid[jidNormalizedUser(mapping.pn)] = mapping.lid;
                                }
                            }
                            participantsList = rawParticipantsList.map(id => {
                                const normalized = jidNormalizedUser(id);
                                return pnToLid[normalized] || id;
                            });
                            logger.debug({ resolved: Object.keys(pnToLid).length, total: pnParticipants.length }, 'Resolved participant JIDs to LIDs for group encryption');
                        }
                        catch (e) {
                            logger.warn({ e }, 'Failed to resolve participant JIDs to LIDs, falling back to JIDs');
                        }
                    }
                }
            ...
            if (typeof content === 'object' &&
                ...
                switch (messageType) {
                    ...
                    case 'INTERACTIVE_BUTTONS': {
                        ...
                        return await relayMessage(jid, ibContent, {
                            messageId: generateMessageIDV2(userJid),
                        });
                    }
                    case 'ALBUM':
